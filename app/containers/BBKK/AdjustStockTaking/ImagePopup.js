...
class ImagePopup extends React.PureComponent {
  ...
  getImgSrc(rowIndex, imgIndex) {
    ...
    if (
      values.cancelBaskets &&
      values.cancelBaskets[rowIndex] &&
      values.cancelBaskets[rowIndex].images[imgIndex]
    ) {
      const img = values.cancelBaskets[rowIndex].images[imgIndex];
      // get img src
      this.setState({
        imgSrc: img.previewData,
      });

      if (img.newlyUploaded) {
        this.setState({
          imgSrc: img.previewData,
        });
      } else {
  ...
  render() {
    const { classes, open, deleteImage, formik } = this.props;
    const { rowIndex, imgIndex, imgSrc } = this.state;
    const { values } = formik;
    const hasNextImage = !!(
      values.cancelBaskets &&
      values.cancelBaskets[rowIndex] &&
      values.cancelBaskets[rowIndex].images[imgIndex + 1]
    );
