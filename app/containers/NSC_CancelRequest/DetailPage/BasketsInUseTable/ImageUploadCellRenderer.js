...
class ImageUploadCellRenderer extends React.Component {
  ...
  uploadHandler = event => {
    ...
    const { formik, rowIndex, node, dispatch } = this.props;
    const { files } = event.target;
    const maxFiles = 3;
    const maxSize = 5e6; // 5 megabytes

    const images =
      formik.values[BASKET_INUSE_TABLE][rowIndex] &&
      formik.values[BASKET_INUSE_TABLE][rowIndex].images
        ? formik.values[BASKET_INUSE_TABLE][rowIndex].images
        : [];
    ...
    Array.from(files).forEach(file => {
      ...
      this.generatePreviewImg(file, previewData => {
        ...
        formik.setFieldValue(
          `${BASKET_INUSE_TABLE}[${rowIndex}]images`,
          updatedImages,
        );
        node.setDataValue(
          `${BASKET_INUSE_TABLE}[${rowIndex}]images`,
  ...
  deleteImage = (rowIndex, imgIndex) => {
    ...
    const { images } = formik.values[BASKET_INUSE_TABLE][rowIndex];
    ...
    if (image.newlyUploaded) {
      ...
      //   : image.id;
      // formik.setFieldValue(
      //   `${BASKET_INUSE_TABLE}[${rowIndex}]imageDeleted`,
      //   updatedImageDeleted,
      // );
    }
  ...
  render() {
    ...
    const keyStr = `${BASKET_INUSE_TABLE}[${rowIndex}]images`;
    const hasError = !!(
      submitCount > 0 &&
      errors[BASKET_INUSE_TABLE] &&
      errors[BASKET_INUSE_TABLE][rowIndex] &&
      errors[BASKET_INUSE_TABLE][rowIndex].images
    );
    const error = hasError ? errors[BASKET_INUSE_TABLE][rowIndex].images : '';
