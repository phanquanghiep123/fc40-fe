...
import { MuiTableBody, MuiTableHeader } from 'components/MuiTable';
...
const showCell = (row, value) =>
  (row.is_after === true &&
    ((isNumber(value) && value > 0 && <b>{formatToCurrency(value)}</b>) ||
      '')) ||
  (isNumber(value) && value > 0 && formatToCurrency(value)) ||
  '';
      ...
      for (let i = 0; i < numberShowColumnDate; i += 1) {
        ...
        fields.forEach(item => {
          newtableColumns.push({
            ...
            render: row => showCell(row, row[`${key}${item.field}`]),
          });
        });
      }
    }
      ...
      <div className={classes.topToolbar}>
        <div className={classes.topToolbarPart}>
          <Typography variant="h6">
            II. Thông Tin Sản Lượng Bán Thành Phẩm Thực Tế
          </Typography>
        </div>
        <div className={classes.topToolbarPart}>
          <Can do={CODE.xemDSLSSLBTP} on={SCREEN_CODE.DSLSSLBTP}>
            <Link
              style={{ textDecoration: 'none' }}
              to="/danh-sach-ghi-nhan-slbtp-thuc-te/phan-bo-lich-su-xu-ly-san-xuat"
            >
              <Button
                type="button"
                variant="contained"
          ...
          <MuiThemeProvider theme={muiTheme}>
            <MaterialTable
              ...
              components={{
                ...
                Header: props => <MuiTableHeader {...props} />,
              }}
