...
class TableSection extends PureComponent {
  onCreate = () => {
    this.props.history.push(
      '/danh-sach-phieu-nhap-khay-sot/tao-phieu-nhap-kho-khay-sot?form=1',
    );
  };
  ...
  onSelectionChange = items => {
    ...
    submittedValues.ids = items.map(item => item.basketDocumentId);
  };
  ...
  onOrderChange = (orderBy, orderDirection) => {
    ...
    if (column && column.field) {
      ...
      } else if (column.field === 'subTypeName') {
        const sortOrder = `${orderDirection === 'asc' ? '' : '-'}subType`;
        this.props.onChangeOrder(submittedValues, sortOrder);
      } else if (column.field === 'dateCreate') {
        const sortOrder = `${orderDirection === 'asc' ? '' : '-'}date`;
        this.props.onChangeOrder(submittedValues, sortOrder);
      } else {
  ...
  render() {
    ...
    return (
      <Paper className={classes.space}>
        <Expansion
          title="II. Thông Tin Phiếu Nhập Khay Sọt"
          noPadding
          className={classes.topToolbar}
          rightActions={
            <Grid className={classes.topToolbarPart}>
              <Can do={CODE.taoDSPNKS} on={SCREEN_CODE.DSPNKS}>
                <Button
                  type="button"
                  color="primary"
